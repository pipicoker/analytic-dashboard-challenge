...
import { lastOrders } from '../assets/data/lastorders'
    ...
    <div className='mt-[14px]'>
        <table className='text-left w-full divide-y'>
            <tr className='text-tableHeading font-medium '>
              ...
              {lastOrders.map((data, index:number) => (
                <tr key={index} className=''>
                  ...
                  <td className='text-[#0D062D] dark:text-[#D9D2F9] font-medium'>${data.amount}</td>

                  {/* checks the status. if the status is paid, it sets the text-[#34CAA5] else  text-[#ED544E]*/}
                  <td className={`${data.status === 'Paid' ? 'text-[#34CAA5]' : 'text-[#ED544E]'}`}>{data.status}</td>

                  <td className='flex items-center gap-[6px] text-[#0D062D] dark:text-[#D9D2F9] text-sm'>
